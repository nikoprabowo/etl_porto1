...
│   ├── McDonald_s_Reviews.csv # **(Not uploaded)** Raw data generated by `get_raw_data.py`
│   └── mcdonalds-store-reviews.zip # **(Not uploaded)** Original ZIP file containing raw data
...
### `raw_data` Directory
- The `raw_data` directory is not included in the repository. 
- You can create this directory and download the raw data by running the `get_raw_data.py` script:
  ```bash
  mkdir raw_data
